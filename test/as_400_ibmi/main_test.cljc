(ns as-400-ibmi.main-test
  "Contract + behavioral test for the as_400_ibmi-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "as_400_ibmi-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'as-400-ibmi.main-test)]
