(ns as-400-ibmi.main
  "Kotodama WASM entrypoint for the As_400_ibmi clean-room actor (L4) — Clojure port.
  ...
  facts (`as_400_ibmi.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "as_400_ibmi")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"as_400_ibmi.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "as_400_ibmi-compat" :tier tier :entities entities} 200])
...
(defn start! [] :as_400_ibmi-compat/ready)
